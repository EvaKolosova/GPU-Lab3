...
void task1_opencl(float* data_a, float* data_b, float* result, size_t SIZE, cl_ulong& start, cl_ulong& end, cl_device_type dtype){
	...
	cl_context context = clCreateContextFromType((NULL == platform) ? NULL : properties, dtype, NULL, NULL, &error);
	...
	cl_program program = clCreateProgramWithSource(context, 1, &matrixMultipleGPU, srclen, &error);
	...
	cl_mem a = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * SIZE * SIZE, NULL, NULL);
	cl_mem b = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * SIZE * SIZE, NULL, NULL);
	cl_mem c = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * SIZE * SIZE, NULL, NULL);

	error = clEnqueueWriteBuffer(queue, a, CL_TRUE, 0, sizeof(float) * SIZE * SIZE, data_a, 0, NULL, NULL);
	if (error != CL_SUCCESS) {std::cout << "Enqueue write buffer data_a failed: " << error << std::endl;}

	error = clEnqueueWriteBuffer(queue, b, CL_TRUE, 0, sizeof(float) * SIZE * SIZE, data_b, 0, NULL, NULL);
	if (error != CL_SUCCESS) {std::cout << "Enqueue write buffer data_b failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 0, sizeof(int), &n);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for n failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 1, sizeof(cl_mem), &a);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for a failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 2, sizeof(cl_mem), &b);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for b failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 3, sizeof(cl_mem), &c);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for c failed: " << error << std::endl;}
	...
	error = clEnqueueNDRangeKernel(queue, kernel, dim, NULL, global, local, 0, NULL, &evt);
	if (error != CL_SUCCESS) {std::cout << "Enqueue failed: " << error << std::endl;}
	...
	clEnqueueReadBuffer(queue, c, CL_TRUE, 0, sizeof(float) * n * n, result, 0, NULL, NULL);
...
void task2_opencl(float* data_a, float* data_b, float* result, size_t K, size_t M, size_t N, cl_ulong& start, cl_ulong& end, cl_device_type dtype){
	...
	cl_program program = clCreateProgramWithSource(context, 1, &matrixMultipleGPU_Optim, srclen, &error);
	if (error != CL_SUCCESS) {std::cout << "Create program failed: " << error << std::endl;}

	error = clBuildProgram(program, 1, &device, buildOpts.c_str(), NULL, NULL);
	if (error != CL_SUCCESS) {
	...
	cl_kernel kernel = clCreateKernel(program, "multi_opt", &error);
	if (error != CL_SUCCESS) {std::cout << "Create kernel failed: " << error << std::endl;}

	cl_mem a = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * M * N, NULL, NULL);
	cl_mem b = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * N * K, NULL, NULL);
	cl_mem c = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * M * K, NULL, NULL);

	error = clEnqueueWriteBuffer(queue, a, CL_TRUE, 0, sizeof(float) * M * N, data_a, 0, NULL, NULL);
	if (error != CL_SUCCESS) {std::cout << "Enqueue write buffer data_a failed: " << error << std::endl;}

	error = clEnqueueWriteBuffer(queue, b, CL_TRUE, 0, sizeof(float) * N * K, data_b, 0, NULL, NULL);
	if (error != CL_SUCCESS) {std::cout << "Enqueue write buffer data_b failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 0, sizeof(int), &m);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for m failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 1, sizeof(int), &n);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for n failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 2, sizeof(int), &k);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for k failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 3, sizeof(cl_mem), &a);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for a failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 4, sizeof(cl_mem), &b);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for b failed: " << error << std::endl;}

	error = clSetKernelArg(kernel, 5, sizeof(cl_mem), &c);
	if (error != CL_SUCCESS) {std::cout << "Set kernel args for c failed: " << error << std::endl;}
	...
	clEnqueueReadBuffer(queue, c, CL_TRUE, 0, sizeof(float) * m * k, result, 0, NULL, NULL);
...
void task3_opencl(float* data_a, float* data_b, float* result, size_t K, size_t M, size_t N, cl_ulong& start, cl_ulong& end, cl_device_type dtype){
	...
	cl_program program = clCreateProgramWithSource(context, 1, &matrixMultiImg, srclen, &error);
	if (error != CL_SUCCESS) {std::cout << "Create program failed: " << error << std::endl;}
	...
	cl_mem a = clCreateImage2D(context, CL_MEM_READ_ONLY, &format, N, N, 0, a, &error);
	if (error != CL_SUCCESS) {std::cout << "Err creating aImage: " << error << std::endl;}

	cl_mem b = clCreateImage2D(context, CL_MEM_READ_ONLY, &format, N, N, 0, b, &error);
	if (error != CL_SUCCESS) {std::cout << "Err creating bImage: " << error << std::endl;}

	cl_mem c = clCreateImage2D(context, CL_MEM_WRITE_ONLY, &format, N, N, 0, c, &error);
	if (error != CL_SUCCESS) {std::cout << "Err creating cImage: " << error << std::endl;}

	error = clSetKernelArg(kernel, 0, sizeof(cl_mem), &a);
	...
	error = clEnqueueWriteImage(queue, a, CL_TRUE, origin, region, 0, 0, data_a, 0, NULL, NULL);
	if (error != CL_SUCCESS) {std::cout << "Enqueue write buffer data_a failed: " << error << std::endl;}

	error = clEnqueueWriteImage(queue, b, CL_TRUE, origin, region, 0, 0, data_b, 0, NULL, NULL);
	if (error != CL_SUCCESS) {std::cout << "Enqueue write buffer data_b failed: " << error << std::endl;}
	...
	error = clEnqueueNDRangeKernel(queue, kernel, dim, offsets, global, local, 0, NULL, &evt);
	if (error != CL_SUCCESS) {std::cout << "Enqueue failed: " << error << std::endl;}

	clWaitForEvents(1, &evt);
	auto finish_gpu = std::chrono::steady_clock::now();

	clEnqueueReadImage(queue, c, CL_TRUE, origin, region, 0, 0, result, 0, NULL, NULL);
